...
cutoff = pd.to_datetime("2024-10-07")  # cutoff date
min_country = 300  # Countries with <300 genotype-fitness data were removed.
date_thresh = pd.to_datetime(
    "2024-10-07"
)  # for loss weighting by variant emergence date
target_strain = "D614G"
...
full_df_name = (
    dir + "input_230829/metadata.representative.all_countries.with_date.v2.with_seq.txt"
)
...
def fasta_to_dataframe(fasta_file_name):
    ...
    seq_df = pd.DataFrame({"target": descriptions, "seq": sequences})
    return seq_df
...
def preprocess_data(df, n_targets, max_length, tokenizer, n_samples=10):
    ...
    n_chunks = 10
    seq_chunks = [
        list(chunk) for chunk in np.array_split(unique_seq_df["seq"].tolist(), n_chunks)
    ]

    encodings = Parallel(n_jobs=-1)(
        delayed(parallel_tokenizer_chunk)(seq_chunk, tokenizer, max_length)
...
antibody_escape_df = antibody_escape_df[
    ~antibody_escape_df["source"].isin(negative_source_l)
]
antibody_escape_df = antibody_escape_df[
    antibody_escape_df["IC50"] < 10
]  # filtering by IC50

antibody_escape_df = antibody_escape_df.rename(
    columns={"target_virus": "target", "mutation": "mutant", "site": "position"}
)

antibody_escape_df = antibody_escape_df[
    antibody_escape_df["target"].isin([target_strain])
]  # filtering by target variant

antibody_escape_df.loc[:, "group"] = (
    antibody_escape_df["target"] + "_" + antibody_escape_df["group"]
)  # group = epitope group; group = task_group (used for balancing los weights)
antibody_escape_df.loc[:, "data_group"] = (
    antibody_escape_df["target"] + "_" + antibody_escape_df["condition"]
)  # condition = mAb type; data_group = task_name

antibody_escape_df.loc[:, "mut_escape_w"] = (
    antibody_escape_df["mut_escape"] * antibody_escape_df["neg_log_IC50"]
)  # weighting by IC50

antibody_escape_df.loc[:, "outcome_pre"] = np.log10(
    antibody_escape_df["mut_escape_w"] + 1
)  # log transformation with pusedo count 1


antibody_escape_df.loc[:, "outcome"] = antibody_escape_df.groupby("group")[
    "outcome_pre"
].transform(
    scale_values
)  # scaling so that 0 and 95 percentile fell within the range 0–1
antibody_escape_df.loc[:, "outcome"] = antibody_escape_df["outcome"].clip(
    0, 1
)  # clipping >1 value to 1
...
antibody_escape_df_selected = antibody_escape_df[
    ["group", "data_group", "outcome", "seq"]
]


# reading, filtering, processing genotype-fitness data###############################
full_df = pd.read_table(full_df_name)
full_df = full_df.groupby("country").filter(
    lambda x: len(x) >= min_country
)  # removing countires with fewer data

full_df.loc[:, "clade"] = np.where(
    full_df["Nextclade_pango"].isin(["BA.2.86", "BA.2.86.1", "JN.1", "JN.2"]),
    "BA.2.86",
    full_df["clade"],
)  # renaming BA.2.86
...
input_df = full_df[
    ["relative_Re", "country", "seq", "date.first", "relative_Re_norm", "clade"]
]

input_df.loc[:, "group"] = (
    "fitness"  # group = task_group (used for balancing los weights)
)
input_df.loc[:, "data_group"] = (
    "fitness_" + input_df["country"]
)  # data_group = task_name

input_df_selected = input_df[
    ["group", "data_group", "relative_Re_norm", "seq", "date.first", "clade", "country"]
]
input_df_selected.columns = [
    "group",
    "data_group",
    "outcome",
    "seq",
...
cat_df.loc[:, "major_group"] = np.where(
    cat_df["group"] == "fitness", "fitness", "immune"
)
...
cat_df.loc[:, "major_group2"] = np.where(
    (cat_df["major_group"] == "fitness") & (cat_df["date.first"] >= date_thresh),
    "fitness_recent",
    np.where(
        (cat_df["major_group"] == "fitness") & (cat_df["date.first"] < date_thresh),
        "fitness",
...
weight_dict["fitness"] = (
    weight_dict["fitness"] * 2
)  # doubling weights for fitness tasks
weight_dict["fitness_recent"] = (
    weight_dict["fitness"] * 1
)  # doubling weights for recent variants
...
cat_df.loc[:, "split_class"] = np.where(
    (cat_df["major_group"] == "fitness"),
    cat_df["data_group"] + "_" + cat_df["clade"],
    cat_df["data_group"],
)
...
cat_df_train, cat_df_val, cat_df_test = stratified_kfold_data_split(
    cat_df, fold_id=fold_id
)
...
input_ids_train, attention_masks_train, outcomes_train, weights_train, idxs_train = (
    preprocess_data(cat_df_train, n_targets, max_length, tokenizer)
)
input_ids_val, attention_masks_val, outcomes_val, weights_val, idxs_val = (
    preprocess_data(cat_df_val, n_targets, max_length, tokenizer)
)
input_ids_test, attention_masks_test, outcomes_test, weights_test, idxs_test = (
    preprocess_data(cat_df_test, n_targets, max_length, tokenizer)
)
...
dataset_train = TextDataset(
    input_ids_train, attention_masks_train, outcomes_train, weights_train
)
dataset_val = TextDataset(input_ids_val, attention_masks_val, outcomes_val, weights_val)
dataset_test = TextDataset(
    input_ids_test, attention_masks_test, outcomes_test, weights_test
)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss = self.weighted_mse_loss(
                masked_regression_output, masked_labels, masked_weights
            )
...
predicted_df = pd.DataFrame({"predicted_outcome": masked_predictions.numpy()})
